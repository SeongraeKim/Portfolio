...
public class ReviewDAO {
	...
	public List allReview(int startrow){
		...
		try {
			...
			while(rs.next()){
			//	System.out.println("DAO 호출 " + rs.getString("idx"));
			...
			System.out.println("allReview 메서드 에러 발생");
			e.printStackTrace();
		} finally {
			closeDB();
		}
 ...
 * 	public void updateReview(int idx, ReviewVO reviewVO) {
		...
		try{
			...
			if(rs.next()){
				sql="update review set title=?, content=?, rating=?, imageFileName=? where idx =?";
				pstmt=con.prepareStatement(sql);
				...
				pstmt.setInt(5, idx);
				pstmt.executeUpdate();
				System.out.println("updateReview 메서드 성공");
			}
...
//			pstmt.executeUpdate();
//			
//			sql="select readcount from review where idx=?";
//			pstmt=con.prepareStatement(sql);
//			pstmt.setInt(1, idx);	
//			rs=pstmt.executeQuery();
//			rs.next();
