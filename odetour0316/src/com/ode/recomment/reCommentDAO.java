...
public class reCommentDAO {
	...
	public List<reCommentVO> allComment(int parentNum, int commentRealPage) {
		...
		try {
			...
			System.out.println("allComment 메서드 오류 발생 ");
			e.printStackTrace();
		} finally{
			closeDB();
		}
	...
	public void insertComment(reCommentVO recommentVO) {
		...
		try {
			...
			System.out.println("insertComment 메서드 에러 발생");
			e.printStackTrace();
		} finally {
			closeDB();
		}
	...
	public int updateComment(int idx , String content) {
		try {
			...
			System.out.println("updateComment 메서드 에러 발생");
			e.printStackTrace();
		} finally {
			closeDB();
		}
	...
	public int deleteComment(int idx){
		try {
			...
			System.out.println("deleteComment 메서드 에러 발생");
			e.printStackTrace();
		} finally {
			closeDB();
		}
	...
	private int getNewCommentIdx(){
		...
		try {
			...
			System.out.println("getNewCommentIdx 메서드 에러 발생");
			e.printStackTrace();
		} finally {
			closeDB();
		}
	...
	public int getAllCommentSize(int parentNum) {
		...
		try {
			...
			System.out.println("getAllCommentSize 메서드 에러 발생");
			e.printStackTrace();
		} finally{
			closeDB();
		}
	...
	public List<Integer> getCommentSize(List<ReviewVO> reviewList) {
		...
		try {
			con=getConnection();
			sql = "select parentnum from reviewcomment";
			pstmt =con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next()){
				int count = 0;
				for(ReviewVO reviewVO : reviewList){
					if(reviewVO.getIdx() == rs.getInt("parentNum")){
						count++;
					}
					commentSizeList.add(count);
				}
			}
